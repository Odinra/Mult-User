	...
	case WM_COMMAND:
		{
			...
			{			
				...
				if(Message[0] != '\0')
				{
					...
					SendBytes = send(skServer, Message, sizeof(SendData), 0);
				}
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR lpcmdline, int ncmdshow)
{
	...
	if(!(hwnd = CreateWindowEx(NULL, WINDOW_CLASS_NAME, "BattleShip - CLIENT", WS_OVERLAPPED|WS_SYSMENU|WS_VISIBLE,
	...
	if(horriz)
	{
		...
		if(r - 11 + s < 10)
		{
			...
			for(int size = ShipSize(); size > 0; size--)
			{
				...
				{
					FillBox(boxes[r][c]->left - 5 + ((size-2) * 40), boxes[r][c]->top + 5, boxes[r][c]->right - 5 + ((size-2) * 40), boxes[r][c]->bottom + 5, SHIPCOLOR);
